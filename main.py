...
def main(args):
    ...
    if args.eval is False:
        logger = create_logger(output_dir=args.output_dir, dist_rank=0)
    ...
    if args.eval:
        ...
        ckpt = torch.load(args.eval_model_path)["model"]
        model_dict = model.state_dict()
        ckpt = {k: v for k, v in ckpt.items() if k in model_dict}
        model.load_state_dict(ckpt)
        test_stats = evaluate(data_loader_val, model, device, use_amp=args.use_amp, logger=logger, update_freq=args.update_freq)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            if args.model_ema and args.model_ema_eval:
                test_stats_ema = evaluate(data_loader_val, model_ema.ema, device, use_amp=args.use_amp, logger=logger, update_freq=args.update_freq)
