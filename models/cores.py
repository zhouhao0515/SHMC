...
class BPP(nn.Module):
    ...
    def forward(self, features1, features2):
        ...
        essential_matrix = (torch.einsum('imjk,injk->imn', (features2, features1)) /(float(H * W)+ 1e-7)).view(B, -1)
        essential_matrix = essential_matrix.clamp(min=-1e3, max=1e3)
        essential_matrix = torch.sign(essential_matrix) * torch.sqrt(
            torch.abs(essential_matrix) + 1e-7  # 使用更大的保护值
        )
        norm = torch.norm(essential_matrix, dim=-1, keepdim=True)
        norm = torch.where(norm < 1e-9, torch.ones_like(norm), norm)
