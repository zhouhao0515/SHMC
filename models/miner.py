...
class ConvNeXt(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        self.intrisics_reduce = nn.Sequential(
            nn.Conv2d(dims[-1], 128, kernel_size=1),
            nn.ReLU(inplace=True)#,
            # nn.AdaptiveAvgPool2d(1)
        )

        self.attention_reduce = nn.Sequential(
            nn.Conv2d(self.M, 128, kernel_size=1),  # 输入通道数为M=16
            nn.ReLU(inplace=True)#,
            #nn.AdaptiveAvgPool2d(1)
        )
        # 新增以降维后intrisics为输入的全连接层分类器
        self.intrisics_head = nn.Linear(128*7*7, self.num_classes)
        # 新增以attention_maps为输入的全连接层分类器
        self.attention_maps_head = nn.Linear(128*7*7, self.num_classes)
    ...
    def forward(self, x):
        ...
        ##addbyzhouhao for multiexperts
        _, _, self.h, self.w = intrisics.size()
        intrisics_reduced = self.intrisics_reduce(intrisics)
        intrisics_flatten = intrisics_reduced.view(intrisics_reduced.size(0), -1)
        intrisics_pred = self.intrisics_head(intrisics_flatten)
        attention_reduced = self.attention_reduce(attention_maps)
        attention_maps_flatten = attention_reduced.view(attention_reduced.size(0), -1)
        attention_maps_pred = self.attention_maps_head(attention_maps_flatten)


        if self.use_ref:
            return pred, intrisics_pred, attention_maps_pred, self.refiner_input(x.size(0), attention_maps)
        else:
            return pred, intrisics_pred, attention_maps_pred, []
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        if self.data_format == "channels_last":
            ...
            s = (x - u).pow(2).mean(1, keepdim=True)+self.eps
